...
for DS in DS_list:
    ...
    exec("desloc_q_A = "+ DS + "_A_D")
    exec("desloc_q_B = "+ DS + "_B_D")
    exec("desloc_q_C = "+ DS + "_C_D")
    Q_CTE_list = [temp_q_A,temp_q_B,temp_q_C]
        
        
    #Faz a leitura da densidade
    if os.path.isfile('IMPORT/EXP_BOM/' + DS + '.csv'):
        print("Resultado experimental localizado e importado")
        with open('IMPORT/EXP_BOM/' + DS + '.csv') as f:
            df = pd.read_csv('IMPORT/EXP_BOM/' + DS + '.csv')
    
        df['pExper'] = df['RETRACAO_MA3'] * P0
        #df['pExper'] = df['RetracaoCubica'] * P0
    ...
    for Q_CTE in Q_CTE_list:
        ...
        temp_celsius = []
        for t in df['TEMPERATURE_MA3']:
            temp_celsius.append(t - 273)
        if converte:
            df['TEMPERATURE_MA3'] = temp_celsius
        df['TEMPERATURE_MA3'] = [x + y for x, y in zip(df['TEMPERATURE_MA3'],[add_temperature]*len(df['TEMPERATURE_MA3']))]
        ...
        #    plotExport(10,temp_y,   pAmostra,   "Linear",   "Temperatura ºC",  "ρAmostra",             "Compararivo de ρ - Calculado e Experimental", 1, df['TEMPERATURE_MA3'], df['pExper'])
        """
        ...
        ax.plot(df['Time'], df['pExper'], label="Experimental")
    ...
    for (pAm, tempo, Q) in zip(Todos_os_pAmostra, Todos_os_tempos, Q_list):
        if n == 0:
            closest = takeClosest(desloc_q_A, pAm)
        elif n == 1:
            closest = takeClosest(desloc_q_B, pAm)
        else:
            closest = takeClosest(desloc_q_C, pAm)
        ...
        ax.plot(tempo, pAm, label=str(Q) + " ºC/min", linewidth=8)
    
    df['pExper'] = df['pExper'].replace(np.nan, P0)
    tExp = list(df['TIME_MA3']/1000)
    pExp = list(df['pExper'])

    
    closest = takeClosest(0.78, pExp)
    index = pExp.index(closest)
    tExp[:] = [t - tExp[index] for t in tExp]
    ax.plot(tExp, pExp, label="Experimental", linestyle='dotted', linewidth=5)
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig("Export/_" + DS + ".png", dpi = 100)
    plt.close(fig)
    ...
    for (pAm, tempo, Q) in zip(Todos_os_pAmostra, Todos_os_tempos, Q_list):
        ...
        #TODO CHECAR SE É DIVIDIDO MESMO
        tempo[:] = [t - tempo[index] for t in tempo]
        ax.plot(tempo, pAm, label=str(Q) + " ºC/min", linewidth=5)
    ...
    plt.gcf().set_size_inches(16, 9)
    plt.xlim([-10, 4])
    plt.axvline(x=-1.4, linewidth=3, color='black')
    plt.axvline(x=0, linewidth=3, color='black')
    plt.tight_layout()
    plt.show()
    fig.savefig("Export/_" + DS + "_zoom.png", dpi = 100)
    plt.close(fig)