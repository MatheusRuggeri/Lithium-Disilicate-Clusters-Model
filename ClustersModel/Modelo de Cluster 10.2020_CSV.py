...
if os.path.isfile('IMPORT/' + AMOSTRA_NAME + '_EXPERIMENTAL_DENSTITY_' + DS + '.txt'):
    ...
    with open('IMPORT/' + AMOSTRA_NAME + '_EXPERIMENTAL_DENSTITY_' + DS + '.txt') as f:
        Lines =             f.readlines()
        TExper_Import =     [line.split()[0] for line in Lines]
        pExper_Import =     [line.split()[1] for line in Lines]
        
    TExper = [float(i) for i in TExper_Import]
    pExper = [float(i) for i in pExper_Import]

    "Define a densidade inicial como a primeira experimental"
    P0 = pExper[0];
else:
...
'''
if os.path.isfile('IMPORT/' + DS + '.csv'):
    df = pd.read_csv('IMPORT/' + DS + '.csv')
    TExper = list(df['TEMPERATURE'] + 273.15 + 20) # Soma kelvin e muda para temp do forno
    retExper = list(abs(df['DISTANCE']-df['DISTANCE'][0])/df['DISTANCE'][0])
    pExper = list(df['RETRACAO']*P0)

#Cria o diretório de exportação e limpa a variavel Lines (já foi utilizada)
if not os.path.exists("Export/" + AMOSTRA_NAME):
    os.mkdir("Export/" + AMOSTRA_NAME)
del Lines
...
def plotExport(n, xPlot, yPlot, typePlot, xLeg, yLeg, titulo, show, secondXPlot=0, secondYPlot=0):
    ...
    fig.savefig("Export/" + AMOSTRA_NAME + "/" + str(n) + " - " + titulo + ".png", dpi = 300, bbox_inches ='tight')
    if show:
        plt.show()
    plt.close(fig)
...
if converte:
    TExper = temp_celsius
TExper = [x + y for x, y in zip(TExper,[add_temperature]*len(TExper))]
...
# Retração
plotExport(8, temp_y,   dl,  "Linear",  "Temperatura ºC",  "Retração",  "Retração - Calculado e Experimental", 1, TExper, retExper)


if os.path.isfile('IMPORT/' + AMOSTRA_NAME + '_EXPERIMENTAL_DENSTITY_' + DS + '.txt'):
    plotExport(10,temp_y,   pAmostra,   "Linear",   "Temperatura (K)",  "ρAmostra",             "Compararivo de ρ - Calculado e Experimental", 1, TExper, pExper)

# Comparando 3 modelos e experimental
fig, ax = plt.subplots()
ax.plot(temp_y, pAmostraFr, label="Frenkel Puro")
ax.plot(temp_y, pAmostraMS, label="MS Puro")
ax.plot(temp_y, pAmostra, label="Clusters")
ax.plot(TExper, pExper, label="Experimental")
ax.set(xlabel="Temperatura", ylabel="ρAmostra", title="ρ Amostra para cada Visc")
ax.grid()
ax.legend(loc="upper right", prop={'size': 8})
plt.gcf().set_size_inches(8, 5)
fig.savefig("Export/" + AMOSTRA_NAME + "/12 - Compara 3 modelos.png", dpi = 300)
#plt.show()
plt.close(fig)