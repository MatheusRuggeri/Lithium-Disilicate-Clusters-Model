...
while Q_CTE < 1001:
    ...
    tempo  = [(i - Tg_CTE) for i in list(range(Tg_CTE, Tm_CTE, Step_CTE))]
    ...
    if os.path.isfile('IMPORT/' + AMOSTRA_NAME + '_EXPERIMENTAL_DENSTITY_' + DS + '.txt'):
        plotExport(10,temp_y,   pAmostra,   "Linear",   "Temperatura ºC",  "ρAmostra",             "Compararivo de ρ - Calculado e Experimental", 1, TExper, pExper)
    ...
    plt.gcf().set_size_inches(8, 5)
    fig.savefig("Export/" + AMOSTRA_NAME + "/Taxa_" + str(Q_CTE) + ".png", dpi = 300)
    #plt.show()
    plt.close(fig)